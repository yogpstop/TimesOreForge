...
public class GuiOre extends GuiScreen {
	...
	private void getValue() {
		byte dIndex = Default.get(TimesOreForge.setting.get(ore).baseBlock), dBlocks = Default.blocks(dIndex);

		lapis.displayString = StatCollector.translateToLocal("tof.likelapis")
				+ ": "
				+ (TimesOreForge.setting.get(ore).likeLapis == true ? StatCollector.translateToLocal("options.on") : StatCollector
		...
		if (TimesOreForge.setting.get(ore).isSetMultiple == 1 || TimesOreForge.setting.get(ore).isSetMultiple == 2) {
			TimesOreForge.setting.get(ore).setBlocks(dBlocks);
		}
		maxHeight.displayString = StatCollector.translateToLocal("tof.maxheight") + ": " + Integer.toString(TimesOreForge.setting.get(ore).height);
		int blocks = TimesOreForge.setting.get(ore).lumps * TimesOreForge.setting.get(ore).getBlocks();
		float density = (float) blocks / (float) TimesOreForge.setting.get(ore).height;
		int dblocksa = 0;
		float ddensitya = 0;
		byte Index;
		if ((Index = Default.get(TimesOreForge.setting.get(ore).blockID)) != -1 && TimesOreForge.setting.get(ore).meta == 0) {
		...
		this.blocks.displayString = StatCollector.translateToLocal("tof.blocksperchunk") + ": " + Integer.toString(blocks);

		int dblocks = Default.lumps(dIndex) * dBlocks;
		float ddensity = (float) dblocks / (float) Default.height(dIndex);
		baseBlock.displayString = Block.blocksList[TimesOreForge.setting.get(ore).baseBlock].getLocalizedName() + StatCollector.translateToLocal("tof.base");

		switch (TimesOreForge.setting.get(ore).isSetMultiple) {
		case 0:
			maxLumpsPerChunk.displayString = StatCollector.translateToLocal("tof.lumps") + ": " + Integer.toString(TimesOreForge.setting.get(ore).lumps);
			maxBlocksPerLump.displayString = StatCollector.translateToLocal("tof.blocks") + ": " + Integer.toString(TimesOreForge.setting.get(ore).getBlocks());
			for (int i = 6; i <= 11; i++) {
		...
		case 1:
			...
			maxLumpsPerChunk.displayString = StatCollector.translateToLocal("tof.ratio") + ": " + (cache1.length() > 7 ? cache1.substring(0, 6) : cache1);
			maxBlocksPerLump.displayString = StatCollector.translateToLocal("tof.densitybase");
			for (int i = 6; i <= 11; i++) {
		...
		case 2:
			...
			maxLumpsPerChunk.displayString = StatCollector.translateToLocal("tof.ratio") + ": " + (cache3.length() > 7 ? cache3.substring(0, 6) : cache3);
			maxBlocksPerLump.displayString = StatCollector.translateToLocal("tof.blocksbase");
			for (int i = 6; i <= 11; i++) {
				moveB[i].enabled = false;
			}
			break;
	...
	private void addButton() {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 6; j++) {
				moveB[i * 6 + j] = new GuiButton(10 * i + j, this.width / 2 + pos[j], 20 * i + 20, size[j], 20, moveb[j]);
				buttonList.add(moveB[i * 6 + j]);
			}
		}

		maxLumpsPerChunk = new GuiButton(6, this.width / 2 - 128, 20, 150, 20, "");
		maxLumpsPerChunk.enabled = false;
		buttonList.add(maxLumpsPerChunk);

		buttonList.add(new GuiButton(8, this.width / 2 + 80, 20, 100, 20, StatCollector.translateToLocal("tof.toggle")));

		maxBlocksPerLump = new GuiButton(16, this.width / 2 - 128, 40, 150, 20, "");
		maxBlocksPerLump.enabled = false;
		buttonList.add(maxBlocksPerLump);

		maxHeight = new GuiButton(26, this.width / 2 - 128, 60, 150, 20, "");
		maxHeight.enabled = false;
		buttonList.add(maxHeight);

		lapis = new GuiButton(31, this.width / 2 - 180, 80, 170, 20, "");
		buttonList.add(lapis);

		biome = new GuiButton(32, this.width / 2 + 10, 80, 170, 20, StatCollector.translateToLocal("tof.selectbiome"));
		buttonList.add(biome);

		density = new GuiButton(33, this.width / 2 - 180, 100, 170, 20, "");
		density.enabled = false;
		buttonList.add(density);
		...
		buttonList.add(baseBlock);

		buttonList.add(new GuiButton(-1, this.width / 2 - 125, this.height - 26, 120, 20, StatCollector.translateToLocal("gui.done")));

		buttonList.add(new GuiButton(-2, this.width / 2 + 5, this.height - 26, 120, 20, StatCollector.translateToLocal("generator.default")));
	}

	@Override
	public void drawScreen(int i, int j, float k) {
		drawDefaultBackground();
		String title = TimesOreForge.getname(TimesOreForge.setting.get(ore).blockID, TimesOreForge.setting.get(ore).meta) + " "
		...
		fontRenderer.drawStringWithShadow(StatCollector.translateToLocal("tof.chunkinfo"), 40, 130, 0xFFFFFF);
		fontRenderer.drawStringWithShadow(StatCollector.translateToLocal("tof.ratioinfo"), 40, 150, 0xFFFFFF);
		fontRenderer.drawStringWithShadow(StatCollector.translateToLocal("tof.ratioinfo2"), 40, 170, 0xFFFFFF);
		fontRenderer.drawStringWithShadow(StatCollector.translateToLocal("tof.baseblockinfo"), 40, 190, 0xFFFFFF);
	...
	public void actionPerformed(GuiButton par1) {
		...
		case -2:
			...
			if (Default.get(TimesOreForge.setting.get(ore).blockID) != -1) {
				TimesOreForge.setting.get(ore).baseBlock = TimesOreForge.setting.get(ore).blockID;
			}
			if (TimesOreForge.setting.get(ore).baseBlock == Block.oreEmerald.blockID) {
				TimesOreForge.setting.get(ore).allBiome = false;
				TimesOreForge.setting.get(ore).biomes.add(BiomeGenBase.extremeHills.biomeID);
				TimesOreForge.setting.get(ore).biomes.add(BiomeGenBase.extremeHillsEdge.biomeID);
			}
			...
			TimesOreForge.setting.get(ore).lumps = 0;
			TimesOreForge.setting.get(ore).likeLapis = (TimesOreForge.setting.get(ore).blockID == Block.oreLapis.blockID ? true : false);
			getValue();
			break;
		case 8:
		...
		case 3:
			...
			break;
		case 31:
			TimesOreForge.setting.get(ore).likeLapis = (TimesOreForge.setting.get(ore).likeLapis == true ? false : true);
			getValue();
			break;
		case 32:
			TimesOreForge.setting.get(ore).closeGui();
